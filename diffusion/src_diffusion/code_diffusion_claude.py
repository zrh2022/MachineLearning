...
import math  # 导入数学库
import os  # 导入操作系统接口import matplotlib.pyplot as plt
...
def train_ddpm(model, train_loader, num_epochs=100, lr=2e-4):
    ...
    for epoch in range(num_epochs):  # 遍历每个epoch
        ...
        # 每10个epoch保存一次模型
        if (epoch + 1) % 1 == 0:  # 每10个epoch
            os.makedirs('checkpoints', exist_ok=True)  # 创建保存目录
            torch.save(model.model.state_dict(), f'checkpoints/ddpm_epoch_{epoch + 1}.pth')  # 保存模型
            print(f"模型已保存: checkpoints/ddpm_epoch_{epoch + 1}.pth")  # 提示保存成功
...
def load_and_inference(model_path, num_samples=4):
    ...
    # 加载训练好的权重
    ddpm.model.load_state_dict(torch.load(model_path, map_location=device))  # 加载模型权重
    print(f"模型已从 {model_path} 加载")  # 提示加载成功
...
def save_model_checkpoint(model, optimizer, epoch, loss, filepath):
    ...
    os.makedirs(os.path.dirname(filepath), exist_ok=True)  # 创建目录
    torch.save(checkpoint, filepath)  # 保存检查点
    print(f"检查点已保存到: {filepath}")  # 提示保存成功
...
def main():
    ...
    # 训练模型
    print("开始训练...")  # 提示开始训练
    train_ddpm(ddpm, train_loader, num_epochs=200000, lr=2e-4)  # 训练模型
...
def simple_training_demo():
    ...
    # 快速训练几个epoch
    optimizer = optim.Adam(ddpm.model.parameters(), lr=1e-3)  # 创建优化器
    ddpm.model.train()  # 设置训练模式

    for epoch in range(3):  # 训练3个epoch
        total_loss = 0  # 总损失
        for batch_idx, (images, _) in enumerate(synthetic_loader):  # 遍历批次
            images = images.to(device)  # 移到GPU

            loss = ddpm.train_step(images)  # 计算损失
        ...
        print(f"Epoch {epoch + 1}, 平均损失: {total_loss / len(synthetic_loader):.4f}")  # 打印损失
...
def model_components():
    ...
    # 测试时间嵌入
    print("测试时间嵌入...")  # 提示
    time_emb = SinusoidalPositionEmbedding(32)  # 创建时间嵌入
    time = torch.randint(0, 1000, (batch_size,))  # 随机时间步
    emb_out = time_emb(time)  # 计算嵌入
    print(f"时间嵌入: {time.shape} -> {emb_out.shape}")  # 打印形状变化

    # 测试残差块
    print("测试残差块...")  # 提示
    res_block = ResidualBlock(64, 128, 128)  # 创建残差块
    x = torch.randn(batch_size, 64, 16, 16)  # 测试输入
    time_emb = torch.randn(batch_size, 128)  # 时间嵌入
    res_out = res_block(x, time_emb)  # 前向传播
    print(f"残差块: {x.shape} + {time_emb.shape} -> {res_out.shape}")  # 打印形状变化

    # 测试注意力块
    print("测试注意力块...")  # 提示
    attn_block = AttentionBlock(128)  # 创建注意力块
    x = torch.randn(batch_size, 128, 8, 8)  # 测试输入
    attn_out = attn_block(x)  # 前向传播
    print(f"注意力块: {x.shape} -> {attn_out.shape}")  # 打印形状变化

    # 测试完整UNet
    print("测试完整UNet...")  # 提示
    unet = UNet()  # 创建UNet
    x = torch.randn(batch_size, 3, 32, 32)  # 测试输入
    time = torch.randint(0, 1000, (batch_size,))  # 时间步
    unet_out = unet(x, time)  # 前向传播
    print(f"UNet: {x.shape} + {time.shape} -> {unet_out.shape}")  # 打印形状变化

    print("所有组件测试通过！")  # 提示测试完成


# 如果直接运行此脚本
# if __name__ == "__main__":
#     # 首先测试组件
#     model_components()  # 测试模型组件
#
#     print("\n" + "=" * 60 + "\n")  # 分隔线
#
#     # 可视化噪声调度
#     visualize_noise_schedule()  # 可视化噪声调度
#
#     print("\n" + "=" * 60 + "\n")  # 分隔线
#
#     # 运行主程序
